...
def get_exchange():
    ...
    if response.status_code == 200:
        data = response.json()
        nm = next((item["cur_nm"] for item in data if item["cur_unit"] == "EUR"), None)
        ttb = next((item["ttb"] for item in data if item["cur_unit"] == "EUR"), None)
        tts = next((item["tts"] for item in data if item["cur_unit"] == "EUR"), None)
